...
export default function EditButton ({ editBtnRef }) {
  const { options, dispatch, mode, activePanel, previousPanel, isMobile } = useApp()
  const { id } = options
  ...
  const button = () => {
    return (
      <button onClick={handleClick} className='fm-c-btn fm-c-btn--edit govuk-body-s' ref={editBtnRef} aria-expanded={false} {...isMobile ? { 'aria-labelledby': `${id}-edit-label` } : {}} {...!isVisible ? { style: { display: 'none' } } : {}}>
  ...
  return (
    <>
      {isMobile
        ? (
          <Tooltip id={`${id}-edit-label`} position='right' cssModifier='edit' text='Edit' display={!isVisible ? 'none' : 'block'}>
            {button()}
          </Tooltip>
          )
        : (
