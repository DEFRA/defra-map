...
export default function SearchButton ({ searchBtnRef, tooltip }) {
  const { dispatch, activePanel, options } = useApp()
  const { id } = options
  ...
  const button = () => {
    return (
      <button onClick={handleClick} className='fm-c-btn fm-c-btn--search govuk-body-s' {...tooltip ? { 'aria-labelledby': `${id}-search-label` } : {}} aria-expanded={activePanel === 'SEARCH'} {...activePanel === 'SEARCH' ? { style: { display: 'none' } } : {}} ref={searchBtnRef}>
