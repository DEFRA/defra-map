...
export default function EditButton ({ editBtnRef }) {
  ...
  const button = () => {
    return (
      <button onClick={handleClick} className='fm-c-btn fm-c-btn--edit govuk-body-s' ref={editBtnRef} aria-expanded={false} {...isMobile ? { 'aria-labelledby': `${id}-edit-label` } : {}} {...activePanel === 'EDIT' ? { style: { display: 'none' } } : {}}>
  ...
  return (
    <>
      {isMobile
        ? (
          <Tooltip id={`${id}-edit-label`} position='right' cssModifier='edit' text='Edit'>
            {button()}
          </Tooltip>
          )
        : (
