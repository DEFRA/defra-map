...
export default function DrawConstraint () {
  ...
  return (
    <div className={`${!dimensions.warningText ? 'fm-u-visually-hidden' : 'fm-c-panel fm-c-panel--draw-constraint'}`} role='status'>
      {dimensions?.area > 0 && (
        <>
          ...
          <span {...({ dangerouslySetInnerHTML: { __html: dimensions?.warningText } })} />
        </>
      )}
    </div>
  )
