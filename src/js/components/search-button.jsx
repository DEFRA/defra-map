...
export default function SearchButton ({ searchBtnRef, tooltip }) {
  ...
  const button = () => {
    return (
      <button onClick={handleClick} className='fm-c-btn fm-c-btn--search govuk-body-s' {...tooltip ? { 'aria-labelledby': `${options.id}-search-label` } : {}} aria-expanded={activePanel === 'SEARCH'} {...activePanel === 'SEARCH' ? { style: { display: 'none' } } : {}} ref={searchBtnRef}>
