...
export const MapStyles = ({ mapState, pluginConfig, services, mapProvider }) => {
  ...
  return (
    <div className='dm-c-map-styles'>
      <div className='dm-c-map-styles__group'>
        <div className='dm-c-map-styles__inner'>
          {mapStyles.filter(mapStyle => mapStyle.url).map(mapStyle => (
            <div className='dm-c-map-styles__item' key={mapStyle.id}>
              <button className='dm-c-map-styles__button' aria-pressed={mapStyle.id === currentMapStyle.id} onClick={() => handleMapStyleClick(mapStyle)}>
                <div className='dm-c-map-styles__image'>
      ...
      {supportsMapSizes && (
        <div className='dm-c-map-styles__group'>
          <h3 className='dm-c-map-styles__heading' id='map-text-sizes'>Text size</h3>
          <div className='dm-c-map-styles__inner'>
            {['small', 'medium', 'large'].map(size => (
              <div className='dm-c-map-styles__item' key={size}>
                <button className='dm-c-map-styles__button' onClick={() => handleMapSizeClick(size)} aria-pressed={size === currentMapSize}>
                  <div className='dm-c-map-styles__image'>
                    <svg width='60' height='60' viewBox='0 0 60 60' fillRule='evenodd'>
                      <rect className='dm-c-map-styles__image-bg' width='100%' height='100%' />
